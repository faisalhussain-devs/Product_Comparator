...
def run_rank_batches(session, inputs, batch_size=128):
    ...
    for i in tqdm(range(0, N, batch_size)):
        ...
        out = session.run(None, batch_inputs)
...
def run_absa_batches(session, inputs, batch_size=300):
    ...
    return (
    np.concatenate(top_logits),
    np.concatenate(sub_logits),
    np.concatenate(comp_logits),
    np.concatenate(sent_logits)
)


for input in inputs:
  top_logits, sub_logits, comp_logits, sent_logits = run_absa_batches(session_absa, input, batch_size=128)

  top_logits_pr.append(top_logits)
  sub_logits_pr.append(sub_logits)
  comp_logits_pr.append(comp_logits)
  sent_logits_pr.append(sent_logits)
...
sent = weighted_sentiment_score(sent_preds, pred_sub_bin, weights_norm)
sentiments = robust_sign(sent) # here sentiments are the final sentiments predicted using weighted technique
conf= final_confidence(usefulness, pred_sub_bin, sentiments, weights, # whereas sent_preds are the prediction of ealier raw sentiments
...
input_llm["product_summary"] = [{"top_aspect_name": nm, "sub_aspects": [],
                                 "specifications": get_specs(i, data_specs)} for i, nm in top_labels.items()]
